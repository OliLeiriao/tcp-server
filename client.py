...
host = socket.gethostname() # Specify this client's host IP
port = 444 # specifies port to use for establishing connection to server

clientsocket.connect((host,port)) # Adds these specifications to our socket object

message = clientsocket.recv(1024) # stores the message received from server, int specifies buffer size limit we will accept for messages from server
